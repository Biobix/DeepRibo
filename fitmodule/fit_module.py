    ...
    def fit(self,
        ...
        for t in range(initial_epoch, epochs):
            ...
            for batch_i, batch_data in enumerate(train_data):
                # Get batch data
                if RNN:
                    X_batch = Variable(batch_data[0].transpose_(0,1))
                else:
                ...
                opt.zero_grad()
                if RNN:
                    hidden = self.init_hidden(len(X_batch[0]))
            ...
            # Run metrics
            if metrics:
                y_train_pred = self.predict(X, batch_size, RNN=RNN)
                add_metrics_to_log(log, metrics, y, y_train_pred)
            if X_val is not None and y_val is not None:
                y_val_pred = self.predict(X_val, batch_size, RNN=RNN)
            ...
            if custom_metric:
                cust_metric = custom_metric(self)
        return logs

    def predict(self, X, batch_size=32, RNN=False):
